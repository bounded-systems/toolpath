...
A Toolpath path's optional `meta.kind` is a URI naming a *kind specification*: a contract describing the additional shape the path follows on top of the base format. Consumers that recognize the URI may rely on the structure that spec describes. Unrecognized URIs should be treated as a generic path.
